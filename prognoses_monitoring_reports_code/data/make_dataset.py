...
def make_dataset(month, data_file_location):
    ...
    df["Business day"] = pd.to_datetime(
        df["Business day"].astype(str) + " " + df["PTE Start time"].astype(str),
        utc=False,
    )
    ...
    df_month["relative ABS Error (DA)"] = (
        df_month["ABS Error (DA)"] / (df_month["Total range [MW]"])
    ) * 100
    df_month["relative ABS Error (ID)"] = (
        df_month["ABS Error (ID)"] / (df_month["Total range [MW]"])
    ) * 100
