...
def dist_errors(df_month_party):
    ...
    plt.hist([df_month_party['Error (DA)'].dropna(),df_month_party['Error (ID)'].dropna()],bins=20, color=['b','g'])
...
def rMAE_marketparty_month(df_month_party):    
    df_temp = df_month_party.groupby(['Business day']).mean()
    fig = plt.figure()
    plt.plot(df_temp['relative ABS Error (DA)'].dropna(), label='DA',marker='o',color='b')
    plt.plot(df_temp['relative ABS Error (ID)'].dropna(), label='ID',marker='o',color='g')
...
def connectionpoint_plot(df_temp,i):   
    ...
    ax2.hist([df_temp['Error (DA)'].dropna(),df_temp['Error (ID)'].dropna()],label=['DA', 'ID'],bins=20, color=['b','g'])
